...
from CTFd.models import db, Users, Solves, Awards, Containers, Challenges, WrongKeys, Keys, Tags, Files, Tracking, Pages, Config, Teams, DatabaseError, get_standings
...
def admin_config():
    if request.method == "POST":
        ...
        team_limit = set_config("team_limit", request.form.get('team_limit', None))
    ...
    team_limit = get_config('team_limit')
...
def admin_chals():
    if request.method == 'POST':
        ...
        teams_with_points = db.session.query(Solves.userid).join(Users).filter(
            Users.banned == False).group_by(
                Solves.userid).count()

        json_data = {'game':[]}
        for x in chals:
            solve_count = Solves.query.join(Users, Solves.userid == Users.id).filter(Solves.chalid == x[1], Users.banned == False).count()
...
def admin_teams(page):
    ...
    teams = db.session.query(Teams, Users).join(Users, Teams.captain == Users.id).order_by(Teams.id.asc()).slice(page_start, page_end).all()
    count = db.session.query(db.func.count(Teams.id)).first()[0]
    pages = int(count / results_per_page) + (count % results_per_page > 0)
    return render_template('admin/teams.html', teams=teams, pages=pages, curr_page=page)


@admin.route('/admin/team/<teamid>/delete', methods=['GET', 'POST'])
@admins_only
def admin_team_delete(teamid):
    try:
        team = Teams.query.filter_by(id=teamid).first()
        users = Users.query.filter_by(teamid=team.id).all()
        for user in users:
            user.teamid = None
...
def admin_team(teamid):
    ...
    users = Users.query.filter_by(teamid=teamid)
    secret = urllib.quote_plus(s.sign(str(team.id)).encode('base64'))

    if request.method == 'GET':
        return render_template('admin/team.html', team=team, users=users, secret=secret)
    elif request.method == 'POST':
...
@admin.route('/admin/user/<userid>', methods=['GET', 'POST'])
@admins_only
...
@admin.route('/admin/user/<userid>/mail', methods=['POST'])
@admins_only
def email_user(userid):
    message = request.form.get('msg', None)
    team = Users.query.filter(Users.id == userid).first()
    if message and team:
...
@admin.route('/admin/user/<userid>/ban', methods=['POST'])
@admins_only
def user_ban(userid):
    user = Users.query.filter_by(id=userid).first()
    user.banned = True
...
@admin.route('/admin/user/<userid>/unban', methods=['POST'])
@admins_only
def unban(userid):
    user = Users.query.filter_by(id=userid).first()
    user.banned = False
...
@admin.route('/admin/user/<userid>/delete', methods=['POST'])
@admins_only
def delete_team(userid):
    try:
        WrongKeys.query.filter_by(userid=userid).delete()
        Solves.query.filter_by(userid=userid).delete()
        Tracking.query.filter_by(user=userid).delete()
...
def admin_graph(graph_type):
    if graph_type == 'categories':
        ...
        solves_sub = db.session.query(Solves.chalid, db.func.count(Solves.chalid).label('solves_cnt')) \
                .join(Users, Solves.userid == Users.id).filter(Users.banned == False) \
...
@admin.route('/admin/users/<userid>/awards', methods=['GET'])
@admins_only
def admin_awards(userid):
...
def admin_scores():
    ...
    teams = db.session.query(Solves.userid, Users.name, score).join(Users).join(Challenges).filter(Users.banned == False).group_by(Solves.userid).order_by(score.desc(), quickest)
    db.session.close()
    json_data = {'teams':[]}
    for i, x in enumerate(teams):
        json_data['teams'].append({'place':i+1, 'id':x.userid, 'name':x.name,'score':int(x.score)})
    return jsonify(json_data)


@admin.route('/admin/solves/<userid>', methods=['GET'])
@admins_only
def admin_solves(userid="all"):
    if userid == "all":
...
@admin.route('/admin/solves/<userid>/<chalid>/solve', methods=['POST'])
@admins_only
def create_solve(userid, chalid):
    solve = Solves(chalid=chalid, userid=userid, ip='127.0.0.1', flag='MARKED_AS_SOLVED_BY_ADMIN')
...
@admins_only
def admin_stats():
    teams_registered = db.session.query(db.func.count(Users.id)).first()[0]
...
def admin_wrong_key(page='1'):
    ...
    wrong_keys = WrongKeys.query.add_columns(WrongKeys.id, WrongKeys.chalid, WrongKeys.flag, WrongKeys.userid, WrongKeys.date, \
                                             Challenges.name.label('chal_name'), Users.name.label('user_name'),
                                             Users.id.label('user_id'), Teams.id.label('team_id'), Teams.name.label('team_name')).\
                join(Challenges).join(Users).join(Teams, Users.teamid == Teams.id).order_by(WrongKeys.date.desc()).slice(page_start, page_end).all()
...
def admin_correct_key(page='1'):
    ...
    solves = Solves.query.add_columns(Solves.id, Solves.chalid, Solves.userid, Solves.date, Solves.flag, \
                                      Challenges.name.label('chal_name'), Users.name.label('user_name'),
                                      Users.id.label('user_id'), Teams.id.label('team_id'),
                                      Teams.name.label('team_name')). \
        join(Challenges).join(Users).join(Teams, Users.teamid == Teams.id).order_by(Solves.date.desc()).slice(page_start, page_end).all()
...
@admin.route('/admin/fails/<userid>', methods=['GET'])
@admins_only
def admin_fails(userid='all'):
    if userid == "all":
        fails = WrongKeys.query.join(Users, WrongKeys.userid == Users.id).filter(Users.banned == False).count()
        solves = Solves.query.join(Users, Solves.userid == Users.id).filter(Users.banned == False).count()
        db.session.close()
